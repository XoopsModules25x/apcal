...
function apcal'.$mydirnumber.'_notify_iteminfo($not_category, $item_id)
{
	...
	if ($not_category=="global") {
		...
		$sql = "SELECT cat_title FROM ".$xoopsDB->prefix("apcal'.$mydirnumber.'_cat")." WHERE cid=\'$item_id\'";
		...
		$sql = "SELECT summary,start FROM ".$xoopsDB->prefix("apcal'.$mydirnumber.'_event")." WHERE id=$item_id";
		$rs = $xoopsDB->query( $sql ) ;
