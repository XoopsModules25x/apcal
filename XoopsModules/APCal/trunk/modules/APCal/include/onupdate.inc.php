...
if( $ref == '' || strpos( $ref , XOOPS_URL.'/modules/system/admin.php' ) === 0 ) {
	...
	// 0.76 to 0.8
	$result = $xoopsDB->query( "SELECT * FROM ".$xoopsDB->prefix("apcal{$mydirnumber}_plugins")." LIMIT 1" ) ;
	if( ! $result ) {
		$xoopsDB->queryF( "ALTER TABLE ".$xoopsDB->prefix("apcal{$mydirnumber}_cat")." CHANGE cat_extkey1 cat_depth TINYINT unsigned NOT NULL DEFAULT 0, ADD cat_style varchar(255) NOT NULL default '', ADD KEY (pid), ADD KEY (weight), ADD KEY (cat_depth)" ) ;

		$xoopsDB->queryF( "CREATE TABLE ".$xoopsDB->prefix("apcal{$mydirnumber}_plugins")." (
	...
	// 0.8RC to 0.8RC2
	$result = $xoopsDB->query( "SELECT pi_weight FROM ".$xoopsDB->prefix("apcal{$mydirnumber}_plugins")." LIMIT 1" ) ;
	if( ! $result ) {
		$xoopsDB->queryF( "ALTER TABLE ".$xoopsDB->prefix("apcal{$mydirnumber}_plugins")." ADD pi_weight smallint(5) unsigned NOT NULL default 0, ADD KEY (pi_weight)" ) ;
	}
