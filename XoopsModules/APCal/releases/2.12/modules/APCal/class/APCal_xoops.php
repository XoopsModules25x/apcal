...
function get_submitter_info( $uid )
{
	...
	if($this->nameoruname == 'uname') {$name = $poster->uname();} 
    else 
    {
...
function assign_event_list( &$tpl , $get_target = '' )
{
	...
	$yrs = mysql_query( "SELECT *,UNIX_TIMESTAMP(dtstamp) AS udtstamp , start, end, allday, start_date, end_date, extkey0 FROM $this->table WHERE $whr" , $this->conn ) ;
	$num_rows = mysql_num_rows( $yrs ) ;

	// �ܥ�����
	$yrs = mysql_query( "SELECT *,UNIX_TIMESTAMP(dtstamp) AS udtstamp , start, end, allday, start_date, end_date, extkey0 FROM $this->table WHERE $whr ORDER BY $order LIMIT $pos,$num" , $this->conn ) ;
