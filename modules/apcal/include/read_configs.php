 ...
 */
	...
	$cal->table = $xoopsDB->prefix( "apcal{$mydirnumber}_event" ) ;
	$cal->cat_table = $xoopsDB->prefix( "apcal{$mydirnumber}_cat" ) ;
    $cal->pic_table = $xoopsDB->prefix( "apcal{$mydirnumber}_pictures" ) ;
	$cal->plugin_table = $xoopsDB->prefix( "apcal{$mydirnumber}_plugins" ) ;
	...
	if( is_object( $xoopsUser ) ) {
		...
		if( $isadmin ) {
			...
			while( $cat = mysql_fetch_object( $rs ) ) {
                ...
                if($cat->canbemain == 1) {$cal->canbemain_cats[intval($cat->cid)] = $cat;}
			}
			...
			// ���̥桼���Υ��ƥ��ꥢ����������
			$sql = "SELECT distinct cid,pid,cat_shorttitle,cat_title,cat_desc,color,ismenuitem,cat_depth,canbemain FROM $cal->cat_table LEFT JOIN ".$xoopsDB->prefix('group_permission')." ON cid=gperm_itemid WHERE gperm_name='apcal_cat' AND gperm_modid='$mid' AND enabled AND gperm_groupid IN $ids4sql ORDER BY weight" ;
			$rs = mysql_query( $sql ) ;
			$cal->categories = array() ;
			while( $cat = mysql_fetch_object( $rs ) ) {
				$cal->categories[intval($cat->cid)] = $cat ;
			...
			if( $users_authority & 256 ) {
				...
				if( $insertable && $gperm_handler->checkRight( 'apcal_global' , 2 , $my_group_ids , $mid ) ) $admission_insert_sql = ',admission=1' ;
				else $admission_insert_sql = ',admission=0' ;

				// �Խ�����
				$editable = $gperm_handler->checkRight( 'apcal_global' , 4 , $my_group_ids , $mid ) ;
				if( $editable && $gperm_handler->checkRight( 'apcal_global' , 8 , $my_group_ids , $mid ) ) $admission_update_sql = ',admission=1' ;
		...
		// �����ȤΥ��ƥ��ꥢ����������
		$sql = "SELECT distinct cid,pid,cat_title,cat_desc,color,ismenuitem,cat_depth,canbemain FROM $cal->cat_table LEFT JOIN ".$xoopsDB->prefix('group_permission')." ON cid=gperm_itemid WHERE gperm_name='apcal_cat' AND gperm_modid='$mid' AND enabled AND gperm_groupid='".XOOPS_GROUP_ANONYMOUS."' ORDER BY weight" ;
		$rs = mysql_query( $sql ) ;
		$cal->categories = array() ;
		while( $cat = mysql_fetch_object( $rs ) ) {
			$cal->categories[ intval( $cat->cid ) ] = $cat ;
