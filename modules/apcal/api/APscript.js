...
function getParams()
{
    ...
    if(!('h' in _GET) || _GET['h'] == '') {_GET['h'] = '';}
    if(!('c' in _GET) || _GET['c'] < 0) {_GET['c'] = 0;}
    if(!('n' in _GET) || _GET['n'] < 1) {_GET['n'] = 10;}
    if(!('w' in _GET) || _GET['w'] == '') {_GET['w'] = '100';}
    if(!('t' in _GET) || _GET['t'] == '') {_GET['t'] = 'default';}
}
...
function addCSS()
{
    ...
    link.rel = 'stylesheet';
    link.href = _GET['t'] == 'theme' ? _GET['URL']+'/modules/APCal/api/APstyle.css' : _GET['URL']+'/modules/APCal/api/APdefault.css';
...
function display(v)
{
    ...
    for(var i in e)
        t += '<dt class='+(i%2 ? 'even' : 'odd')+'>'+e[i].start+'</dt><dd class='+(i%2 ? 'even' : 'odd')+'><a href="'+e[i].link+'" title="'+e[i].summary+'" target="_blank">'+e[i].summary+'</a></dd>';container.innerHTML = s+h+t+d;
}
