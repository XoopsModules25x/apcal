...
//images
$roimageedit = XOOPS_URL.'/modules/APCal/images/regonline/edit.png';
$roimagedelete = XOOPS_URL.'/modules/APCal/images/regonline/delete.png';
$roimagesave = XOOPS_URL.'/modules/APCal/images/regonline/save.png';
$roimagesavemore = XOOPS_URL.'/modules/APCal/images/regonline/savemore.png';
$roimagecancel = XOOPS_URL.'/modules/APCal/images/regonline/cancel.png';
$roimagesend = XOOPS_URL.'/modules/APCal/images/regonline/sendmail.png';
...
if (isset($_GET['op'])) { 
	if( $_GET['op'] == 'show_form_activate') { 
		...
		if (isset($_GET['eventurl'])) $eventurl=$_GET['eventurl']."?smode=".$_GET['smode']."&caldate=".$_GET['caldate'];
		if (isset($_GET['title'])) $event=$_GET['title'];
...
if ($show_form_activate) {
	...
	$query = "SELECT ".$xoopsDB->prefix("apcal_ro_events").".* FROM ".$xoopsDB->prefix("apcal_ro_events")." WHERE ((".$xoopsDB->prefix("apcal_ro_events").".roe_eventid)=$eventid)";
	...
	if( $num_rows == 0 ) {
		...
		$number='0';
		$typeedit='0'; //new
	} else while( $ro_result = mysql_fetch_object( $res ) ) {
	...
	$query = "SELECT ".$xoopsDB->prefix("apcal_ro_notify").".* FROM ".$xoopsDB->prefix("apcal_ro_notify")." WHERE ((".$xoopsDB->prefix("apcal_ro_notify").".ron_eventid)=$eventid)";
	$res=$xoopsDB->query($query);
	$num_rows = mysql_num_rows( $res ) ;
	...
	} else while( $ron_result = mysql_fetch_object( $res ) ) {
		$i=$i+1;
		switch ($i) {
			case 1:
	...
	if ($datelimit>0) $datelimit=date("d.m.Y H:i:s", $datelimit);
    if ($eventdate>0) $eventdate=date("d.m.Y H:i:s", $eventdate);
...
if (isset($_POST['activate_x'])) {
	if (!empty($_POST['eventid'])){
		...
		//default-values
		if ($datelimit=='') $datelimit=0;
		if ($datelimit>0) $datelimit=strtotime($datelimit);
		
		if ($number=='') $number='0';
		
		//insert or update data in table apcal_ro_events
		if( $typeedit == '0' ) {
			$query = "Insert into ".$xoopsDB->prefix("apcal_ro_events")." (roe_submitter, roe_eventid, roe_datelimit, roe_number, roe_date_created) values ($uid, $eventid, $datelimit, $number, ".time().")";
		} else {
			$query = "UPDATE ".$xoopsDB->prefix("apcal_ro_events")." SET ";
			$query .= $xoopsDB->prefix("apcal_ro_events").".roe_submitter = $uid, ";
			$query .= $xoopsDB->prefix("apcal_ro_events").".roe_datelimit = $datelimit, ";
			$query .= $xoopsDB->prefix("apcal_ro_events").".roe_number = $number, ";
            $query .= $xoopsDB->prefix("apcal_ro_events").".roe_date_created = ". time()." ";
			$query .= "WHERE (((".$xoopsDB->prefix("apcal_ro_events").".roe_eventid)=$eventid))";	
		}	      
		$res=$xoopsDB->query($query);
		if(!$res) {
		...
		$query = "UPDATE ".$xoopsDB->prefix("apcal_event")." SET ".$xoopsDB->prefix("apcal_event").".extkey0 = 1 WHERE (((".$xoopsDB->prefix("apcal_event").".id)=$eventid))";	
		$res=$xoopsDB->query($query);
		if(!$res) {
			//echo $query;
			redirect_header($eventurl, 3, _APCAL_RO_ERROR_RO_ACTIVATE);
		...
		if( $typeedit == '1' ) {
			...
			$query = "DELETE ".$xoopsDB->prefix("apcal_ro_notify").".* FROM ".$xoopsDB->prefix("apcal_ro_notify")." WHERE ((".$xoopsDB->prefix("apcal_ro_notify").".ron_eventid)=$eventid)";
			$res=$xoopsDB->query($query);
		}
		if (!$email1=='') {
            $submitter = $xoopsUser->getVar('uid');
			$query = "Insert into ".$xoopsDB->prefix("apcal_ro_notify")." (ron_eventid, ron_email, ron_submitter) values ($eventid, '$email1', $submitter)";
			$res=$xoopsDB->query($query);
			if(!$res) {
                redirect_header($eventurl, 3, _APCAL_RO_ERROR_RO_ACTIVATE);
			}		
		...
		if (!$email2=='') {
			$query = "Insert into ".$xoopsDB->prefix("apcal_ro_notify")." (ron_eventid, ron_email, ron_submitter) values ($eventid, '$email2', $submitter)";
			$res=$xoopsDB->query($query);
			if(!$res) {
				redirect_header($eventurl, 3, _APCAL_RO_ERROR_RO_ACTIVATE);
			}		
		...
		if (!$email3=='') {
			$query = "Insert into ".$xoopsDB->prefix("apcal_ro_notify")." (ron_eventid, ron_email, ron_submitter) values ($eventid, '$email3', $submitter)";
			$res=$xoopsDB->query($query);
			if(!$res) {
				redirect_header($eventurl, 3, _APCAL_RO_ERROR_RO_ACTIVATE);
			}		
		...
		if (!$email4=='') {
			$query = "Insert into ".$xoopsDB->prefix("apcal_ro_notify")." (ron_eventid, ron_email, ron_submitter) values ($eventid, '$email4', $submitter)";
			$res=$xoopsDB->query($query);
			if(!$res) {
				redirect_header($eventurl, 3, _APCAL_RO_ERROR_RO_ACTIVATE);
			}		
		...
		if (!$email5=='') {
			$query = "Insert into ".$xoopsDB->prefix("apcal_ro_notify")." (ron_eventid, ron_email, ron_submitter) values ($eventid, '$email5', $submitter)";
			$res=$xoopsDB->query($query);
			if(!$res) {
				redirect_header($eventurl, 3, _APCAL_RO_ERROR_RO_ACTIVATE);
			}		
...
if (isset($_POST['deactivate_x'])) {
	if (!empty($_POST['eventid'])){
		...
		$query = "DELETE ".$xoopsDB->prefix("apcal_ro_members").".* FROM ".$xoopsDB->prefix("apcal_ro_members")." WHERE ((".$xoopsDB->prefix("apcal_ro_members").".rom_eventid)=$eventid)";
		$res=$xoopsDB->query($query);
		if(!$res) {
		...
		$query = "DELETE ".$xoopsDB->prefix("apcal_ro_events").".* FROM ".$xoopsDB->prefix("apcal_ro_events")." WHERE ((".$xoopsDB->prefix("apcal_ro_events").".roe_eventid)=$eventid)";
		$res=$xoopsDB->query($query);
		if(!$res) {
			redirect_header($url, 3, _APCAL_RO_ERROR_RO_DEACTIVATE);
		}
		...
		$query = "UPDATE ".$xoopsDB->prefix("apcal_event")." SET ".$xoopsDB->prefix("apcal_event").".extkey0 = 0 WHERE (((".$xoopsDB->prefix("apcal_event").".id)=$eventid))";	
		$res=$xoopsDB->query($query);
		if(!$res) {
			redirect_header($url, 3, _APCAL_RO_ERROR_RO_DEACTIVATE);
		} else {
...
if (isset($_REQUEST['form_add'])) {
	if (!empty($_REQUEST['eventid'])){
		...
		$query = "SELECT ".$xoopsDB->prefix("apcal_ro_members").".* ";
		$query .= "FROM ".$xoopsDB->prefix("apcal_ro_members");
            ...
            <table border='0' width='100%'>
                ...
                <table border='1' cellpadding='0' cellspacing='0' width='100%'>
                    <tr>
                        ...
                        if (_APCAL_RO_EXTRAINFO1!='') $ret2 .= "<td class='even'>"._APCAL_RO_EXTRAINFO1."</td>";
                        if (_APCAL_RO_EXTRAINFO2!='') $ret2 .= "<td class='even'>"._APCAL_RO_EXTRAINFO2."</td>";
                        if (_APCAL_RO_EXTRAINFO3!='') $ret2 .= "<td class='even'>"._APCAL_RO_EXTRAINFO3."</td>";
                        if (_APCAL_RO_EXTRAINFO4!='') $ret2 .= "<td class='even'>"._APCAL_RO_EXTRAINFO4."</td>";
                        if (_APCAL_RO_EXTRAINFO5!='') $ret2 .= "<td class='even'>"._APCAL_RO_EXTRAINFO5."</td>";
                         $ret2 .= "
                        <td class='even'>"._APCAL_RO_ACTION."</td>
                    </tr>";
                    while( $member = mysql_fetch_object( $res ) ) {
                        ...
                        $ret2 .= 
                            "<tr>
                                ...
                                if (_APCAL_RO_EXTRAINFO1!='') $ret2 .= "<td class='$classname'>$romextrainfo1</td>";
                                if (_APCAL_RO_EXTRAINFO2!='') $ret2 .= "<td class='$classname'>$romextrainfo2</td>";
                                if (_APCAL_RO_EXTRAINFO3!='') $ret2 .= "<td class='$classname'>$romextrainfo3</td>";
                                if (_APCAL_RO_EXTRAINFO4!='') $ret2 .= "<td class='$classname'>$romextrainfo4</td>";
                                if (_APCAL_RO_EXTRAINFO5!='') $ret2 .= "<td class='$classname'>$romextrainfo5</td>";
                                $ret2 .="
...
if (isset($_POST['add_member_x']) || isset($_POST['add_member_more_x'])){
	...
	if (!empty($_POST['eventid'])){
        ...
        if ($email=='') $email='-';
		if ($extrainfo1=='') $extrainfo1='-';
		if ($extrainfo2=='') $extrainfo2='-';
        if ($extrainfo3=='') $extrainfo3='-';
        if ($extrainfo4=='') $extrainfo4='-';
        if ($extrainfo5=='') $extrainfo5='-';

		//read data from apcal_ro_events
		$query = "SELECT ".$xoopsDB->prefix("apcal_ro_events").".roe_number, roe_datelimit FROM ".$xoopsDB->prefix("apcal_ro_events")." WHERE ((".$xoopsDB->prefix("apcal_ro_events").".roe_eventid)=$eventid)";
		$res=$xoopsDB->query($query);
		$num_rows = mysql_num_rows( $res ) ;
		if( $num_rows == 0 ) {
			$number_allowed = 0;
		...
		if( $number_allowed > 0 ) {			
			...
			$query = "SELECT ".$xoopsDB->prefix("apcal_ro_members").".rom_id FROM ".$xoopsDB->prefix("apcal_ro_members")." WHERE ((".$xoopsDB->prefix("apcal_ro_members").".rom_eventid)=$eventid)";
			$res=$xoopsDB->query($query);
			$num_rows = mysql_num_rows( $res ) ;
			if( $num_rows == 0 ) {
				$number_total = 0;
			} else {
				$number_total = mysql_num_rows( $res ) ;
			}
...
if (isset($_POST['remove_member']) || isset($_POST['remove_member_x'])) {
	if (!empty($_POST['rom_id'])){
		...
		$query = "DELETE ".$xoopsDB->prefix("apcal_ro_members").".* FROM ".$xoopsDB->prefix("apcal_ro_members")." WHERE ((".$xoopsDB->prefix("apcal_ro_members").".rom_id)=$rom_id)";
...
if (isset($_REQUEST['list'])) {
	if (!empty($_REQUEST['eventid'])){
		...
		$title=$summary.' ('.$date.' '.$location.')';
		$query = "SELECT ".$xoopsDB->prefix("users").".uname, ".$xoopsDB->prefix("apcal_ro_members").".* FROM ".$xoopsDB->prefix("users")." INNER JOIN ".$xoopsDB->prefix("apcal_ro_members")." ON ".$xoopsDB->prefix("users").".uid = ".$xoopsDB->prefix("apcal_ro_members").".rom_submitter WHERE (((".$xoopsDB->prefix("apcal_ro_members").".rom_eventid)=$eventid)) ORDER BY ".$xoopsDB->prefix("apcal_ro_members").".rom_date_created";
        ...
        if( $num_rows == 0 ) {
	       ...
	       <table border='1' cellpadding='0' cellspacing='0' width='100%'>
	         <tr>
               ...
               if (!_APCAL_RO_EXTRAINFO1=='') $ret .= "<td class='listeheader'>"._APCAL_RO_EXTRAINFO1."</td>";
               if (!_APCAL_RO_EXTRAINFO2=='') $ret .= "<td class='listeheader'>"._APCAL_RO_EXTRAINFO2."</td>";
               if (!_APCAL_RO_EXTRAINFO3=='') $ret .= "<td class='listeheader'>"._APCAL_RO_EXTRAINFO3."</td>";
               if (!_APCAL_RO_EXTRAINFO4=='') $ret .= "<td class='listeheader'>"._APCAL_RO_EXTRAINFO4."</td>";
               if (!_APCAL_RO_EXTRAINFO5=='') $ret .= "<td class='listeheader'>"._APCAL_RO_EXTRAINFO5."</td>";
               $ret .= "
			...
			while( $member = mysql_fetch_object( $res ) ) {
				...
				if ($line==0) { $classname="odd"; $line=1; } else {	$classname="even"; $line=0; }
				$ret .= 
				"<tr>
                    ...
                    if (!_APCAL_RO_EXTRAINFO1=='') $ret .= "<td class='$classname'>$extrainfo1</td>";
                    if (!_APCAL_RO_EXTRAINFO2=='') $ret .= "<td class='$classname'>$extrainfo2</td>";
                    if (!_APCAL_RO_EXTRAINFO3=='') $ret .= "<td class='$classname'>$extrainfo3</td>";
                    if (!_APCAL_RO_EXTRAINFO4=='') $ret .= "<td class='$classname'>$extrainfo4</td>";
                    if (!_APCAL_RO_EXTRAINFO5=='') $ret .= "<td class='$classname'>$extrainfo5</td>";
            ...
            $query = "SELECT ".$xoopsDB->prefix("users").".email ";
            $query .= "FROM ".$xoopsDB->prefix("users");
            $query .= " WHERE (((".$xoopsDB->prefix("users").".uid)=$uid))";
...
if (isset($_POST['edit_member']) || isset($_POST['edit_member_x'])) {
    if (!empty($_POST['rom_id'])){
		...
		$query = "UPDATE ".$xoopsDB->prefix("apcal_ro_members")." SET ";
		$query .= $xoopsDB->prefix("apcal_ro_members").".rom_firstname = '$firstname', "; 
		$query .= $xoopsDB->prefix("apcal_ro_members").".rom_lastname = '$lastname', "; 
		$query .= $xoopsDB->prefix("apcal_ro_members").".rom_email = '$email', ";
        $query .= $xoopsDB->prefix("apcal_ro_members").".rom_extrainfo1 = '$extrainfo1', "; 
		$query .= $xoopsDB->prefix("apcal_ro_members").".rom_extrainfo2 = '$extrainfo2', ";
        $query .= $xoopsDB->prefix("apcal_ro_members").".rom_extrainfo3 = '$extrainfo3', "; 
        $query .= $xoopsDB->prefix("apcal_ro_members").".rom_extrainfo4 = '$extrainfo4', "; 
        $query .= $xoopsDB->prefix("apcal_ro_members").".rom_extrainfo5 = '$extrainfo5' ";         
		$query .="WHERE (((".$xoopsDB->prefix("apcal_ro_members").".rom_id)='$rom_id'))";
...
if (isset($_POST['ro_notify_all']) || isset($_POST['ro_notify_all_x'])) {
	...
	if (!empty($_POST['url'])){
		...
		$query = "SELECT ".$xoopsDB->prefix("apcal_ro_members").".rom_email, rom_firstname, rom_lastname FROM ".$xoopsDB->prefix("apcal_ro_members")." WHERE (((".$xoopsDB->prefix("apcal_ro_members").".rom_eventid)=$eventid) AND not(rom_email is null))";
