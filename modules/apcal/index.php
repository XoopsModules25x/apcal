 ...
 */
	...
	// �ڡ���ɽ����Ϣ�ν���ʬ��
	if( $action == 'Edit' ) {
        if( is_dir( XOOPS_ROOT_PATH . '/modules/APCal/jscalendar' ) ) {
			// jscalendar in module dir (recommended)
			$jscalurl = XOOPS_URL . '/modules/APCal/jscalendar' ;
    ...
    elseif($action == '')
    {
        ...
        if(isset($cat) && $smode != 'List' && !empty($catNameTitle) && $cal->displayCatTitle) {echo '<h1>'.$catNameTitle.'</h1>';}
        if(isset($cat) && $smode != 'List' && !empty($cat['cat_desc'])) {echo $cat['cat_desc'].'<br /><br />';}
        ...
        if($cal->enablecalmap == 1 && is_array($cal->gmPoints) && !empty($cal->gmPoints))
        {
            ...
            if($smode == 'List') 
            {
                $xoopsTpl->assign('map', $tpl->fetch(XOOPS_ROOT_PATH.'/modules/APCal/templates/googlemap.html'));
            }
            else 
            {
                $tpl->display(XOOPS_ROOT_PATH.'/modules/APCal/templates/googlemap.html');
            }
        }
        ...
        if($cal->enablesharing && $smode != 'List') {echo '<div class="share"><a href="'.XOOPS_URL.'/modules/APCal/shareCalendar.php" title="'._APCAL_SHARECALENDAR.'"><img src="'.XOOPS_URL.'/modules/APCal/images/share.png" /><span style="line-height: 32px; margin-bottom: 15px;">'._APCAL_SHARECALENDAR.'</span></a></div>';}
    }

    if(isset($calDisplay)) {echo $calDisplay;}
