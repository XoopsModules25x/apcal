...
$mod_path = XOOPS_ROOT_PATH.'/modules/'.basename(dirname(__FILE__));
if(!class_exists('APCal')) {require_once($mod_path.'/class/APCal.php');}
...
if($cal->enabletellafriend)
{
    ...
    if(isset($_POST['to']))
    {
        ...
        if($xoopsCaptcha->verify() && ($xoopsUser || isset($_POST['xoopscaptcha']))) {$verify = true;}
    }
    else {$verify = true;}
