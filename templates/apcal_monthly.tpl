...
<table class="month">
    <tr>
        ...
        <{if !$week_start}><td class="dayname" style="color: <{$colors[0]}>; width:<{$widths.Sunday}>; <{$frame_css}>"><{$week_middle_names[0]}></td><{/if}>
        <td class="dayname" style="color: <{$colors[1]}>; width:<{$widths.Monday}>; <{$frame_css}>"><{$week_middle_names[1]}></td>
        <td class="dayname" style="color: <{$colors[2]}>; width:<{$widths.Tuesday}>; <{$frame_css}>"><{$week_middle_names[2]}></td>
        <td class="dayname" style="color: <{$colors[3]}>; width:<{$widths.Wednesday}>; <{$frame_css}>"><{$week_middle_names[3]}></td>
        <td class="dayname" style="color: <{$colors[4]}>; width:<{$widths.Thursday}>; <{$frame_css}>"><{$week_middle_names[4]}></td>
        <td class="dayname" style="color: <{$colors[5]}>; width:<{$widths.Friday}>; <{$frame_css}>"><{$week_middle_names[5]}></td>
        <td class="dayname" style="color: <{$colors[6]}>; width:<{$widths.Saturday}>; <{$frame_css}>"><{$week_middle_names[6]}></td>
        <{if $week_start}><td class="dayname" style="color: <{$colors[0]}>; width:<{$widths.Sunday}>; <{$frame_css}>"><{$week_middle_names[0]}></td><{/if}>
    </tr>
...
<{section name=weeks start=0 loop=6}>
    ...
    <tr>
        ...
        <td class="day" style="background: <{$bgcolor}>; <{$frame_css}>">
            <table width="100%" cellspacing="0" cellpadding="0" style="table-layout:fixed; margin:0;">
                <tr>
                    <td align="left" width="25" style="font-size: 1.3em; font-weight: bold; color:<{$color}>;">
                        <a href="<{$cal->make_cal_link('', 'Daily', $cid, $link)}>"  style="color: inherit;">
                            <{$day}>
                        </a>
                    </td>
                    <td style="overflow: hidden; color: <{$color}>;">
                        <a href="<{$cal->make_cal_link('', 'Monthly', $cid, $link)}>" style="color: inherit;">
                        <{if isset($holidays[$link]) && $holidays[$link] != 1}>
                            <{$holidays[$link]}>
                        <{else}>
                            <img src="<{$images_url}>/spacer.gif" alt="" border="0" width="100%" height="12" />
                        <{/if}>
                        </a>
                    </td>
                    ...
                    <a href="<{$xoops_url}>/modules/APCal/?cid=<{$cid}>&smode=Monthly&action=Edit&caldate=<{$link}>">
                        <img src="<{$images_url}>/addevent.gif" border="0" width="14" height="12" alt="<{$smarty.const._APCAL_MB_APCALADDEVENT}>" />
                    </a>
                    <{/if}>
                    </td>
                </tr>
    ...
    <tr>
        ...
        <td colspan="<{$e[$day][$slot].duration}>" class="event" style="<{$frame_css}>" onmouseover="showBox('<{$id}>');" onmouseout="hideBox('<{$id}>');" onmousemove="moveBox('<{$id}>');">
            <a href="<{$events[$id].link}>" style="border-left-color: <{$cats_color[$event.cat]}>; border-bottom-color: <{$cats_color[$event.cat]}>; background: <{$event_bgcolor}>; color: <{$event_color}>;">
            <{if $events[$id].extkey0 == 1}><img src="<{$ro_image}>" height="12px" alt="<{$smarty.const._APCAL_RO_ONLINE_POSS}>" title="<{$smarty.const._APCAL_RO_ONLINE_POSS}>" /><{/if}>
            <{$events[$id].summary}>
            </a>
                    <{if !$for_print}>
            <div id="<{$id}>" class="tooltip" style="border-color: <{$cats_color[$event.cat]}>;">
                <div class="summary" style="background: <{$cats_color[$event.cat]}>;">
                    <{if $events[$id].extkey0 == 1}><img src="<{$ro_image}>" height="15px" alt="<{$smarty.const._APCAL_RO_ONLINE_POSS}>" title="<{$smarty.const._APCAL_RO_ONLINE_POSS}>" /><{/if}>
                    &nbsp;<{$events[$id].summary}>
                </div>
                <div class="details">
                    <{if $events[$id].picture}><img src="<{$events[$id].picture}>" alt="<{$events[$id].summary}>" title="<{$events[$id].summary}>" /><{/if}>
                    <div class="info">
                        <{if $events[$id].start}><span class="bold"><{$smarty.const._APCAL_BEGIN}>:</span> <{$events[$id].start}><br /><{/if}>
                        <{if $events[$id].end}><span class="bold"><{$smarty.const._APCAL_END}>:</span> <{$events[$id].end}><br /><{/if}>
                        <{if $events[$id].location}><span class="bold"><{$smarty.const._APCAL_LOCATION}>: </span><{$events[$id].location}><br /><{/if}>
                        <br />
                        <div class="click" <{if $events[$id].picture}>style="margin-left: 60px;"<{/if}>><br /><{$smarty.const._APCAL_CLICKFORDETAILS}></div>
                    </div>
                </div>
            </div>
                    <{/if}>
