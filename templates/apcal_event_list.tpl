  ...
  <tr>
    <td class='calframe'>
      ...
      <table border='0' cellspacing='0' cellpadding='0' style='width:100%;'>
             ...
             <div class="socialNetworks">
                ...
                <span class="tellafriend">
                    <a href="" title="<{$smarty.const._APCAL_TELLAFRIEND}>" onclick="window.open('<{$xoops_url}>/modules/APCal/tellafriend.php?url='+encodeURIComponent(location.href)+'&title='+encodeURIComponent(document.title), '<{$smarty.const._APCAL_TELLAFRIEND}>', 'toolbar=no, width=550, height=550'); return false;">
                        <img src="<{$xoops_url}>/modules/APCal/assets/images/tellafriend.png" height="20" width="20" alt="<{$smarty.const._APCAL_TELLAFRIEND}>" title="<{$smarty.const._APCAL_TELLAFRIEND}>" />
                    </a>
                </span>
                <{/if}>
        ...
        <{/if}>
            <div class="share">
                <a href="<{$xoops_url}>/modules/APCal/shareCalendar.php" title="<{$smarty.const._APCAL_SHARECALENDAR}>">
          ...
          <td nowrap='nowrap' align='center' bgcolor='<{$calhead_bgcolor}>'>
            <table border='0' cellspacing='0' cellpadding='0' width='100%' class='apcal-categoryform'>
              <tr>
                ...
                <td width="290" align='center' valign='middle' style='vertical-align:middle;' class='apcal-categoryform'>
                  <{$categories_selform}>
                </td>
                ...
                <{* DATE DESCRIPTION *}>
                <td width='350' align='center' valign='middle' nowrap='nowrap' style='vertical-align:middle;'>
                  <img src='<{$images_url}>/spacer.gif' alt='' width='350' height='4' /><br />
                  <form action='<{$get_target}>?smode=List&amp;num=<{$num}>&amp;cid=<{$now_cid}>&amp;order=<{$order}>' method='POST' style='margin:0px;'>
                    <font size='4' color='<{$calhead_color}>'><span class='calhead'><{$ymdo_selects}><input type='submit' name='apcal_jumpcaldate' value='<{$lang_button_jump}>' /></font></span></font><br />
                  </form>
                ...
                <td align='right' valign='middle' style='vertical-align:middle;'>
                  ...
                  <a href='<{$YEARLYVIEW}>'><img src='<{$images_url}>/year_on.gif' border='0' width='30' height='24' alt='<{$alt_yearly}>' title='<{$alt_yearly}>' /></a>
                  <a href='<{$MONTHLYVIEW}>'><img src='<{$images_url}>/month_on.gif' border='0' width='30' height='24' alt='<{$alt_monthly}>' title='<{$alt_monthly}>' /></a>
                  <a href='<{$WEEKLYVIEW}>'><img src='<{$images_url}>/week_on.gif' border='0' width='30' height='24' alt='<{$alt_weekly}>' title='<{$alt_weekly}>' /></a>
                  <a href='<{$DAILYVIEW}>'><img src='<{$images_url}>/day_on.gif' border='0' width='30' height='24' alt='<{$alt_daily}>' title='<{$alt_daily}>' /></a>
  ...
  <tr>
    <td align="center">
      ...
      <div>
        ...
        <{$lang_sortby}>&nbsp;&nbsp;
        <{$lang_startdatetime}> (<a href="<{$get_target}>?smode=List&amp;cid=<{$now_cid}>&amp;caldate=<{$caldate}>&amp;op=<{$op}>&amp;order=start"><img src="<{$images_url}>/up.gif" border="0" align="middle" alt="" /></a><a href="<{$get_target}>?smode=List&amp;cid=<{$now_cid}>&amp;caldate=<{$caldate}>&amp;op=<{$op}>&amp;order=start+DESC"><img src="<{$images_url}>/down.gif" border="0" align="middle" alt="" /></a>)
        <{$lang_lastmodified}> (<a href="<{$get_target}>?smode=List&amp;cid=<{$now_cid}>&amp;caldate=<{$caldate}>&amp;op=<{$op}>&amp;order=dtstamp"><img src="<{$images_url}>/up.gif" border="0" align="middle" alt="" /></a><a href="<{$get_target}>?smode=List&amp;cid=<{$now_cid}>&amp;caldate=<{$caldate}>&amp;op=<{$op}>&amp;order=dtstamp+DESC"><img src="<{$images_url}>/down.gif" border="0" align="middle" alt="" /></a>)
        <{$lang_summary}> (<a href="<{$get_target}>?smode=List&amp;cid=<{$now_cid}>&amp;caldate=<{$caldate}>&amp;op=<{$op}>&amp;order=summary"><img src="<{$images_url}>/up.gif" border="0" align="middle" alt="" /></a><a href="<{$get_target}>?smode=List&amp;cid=<{$now_cid}>&amp;caldate=<{$caldate}>&amp;op=<{$op}>&amp;order=summary+DESC"><img src="<{$images_url}>/down.gif" border="0" align="middle" alt="" /></a>)
        <{$lang_submitter}> (<a href="<{$get_target}>?smode=List&amp;cid=<{$now_cid}>&amp;caldate=<{$caldate}>&amp;op=<{$op}>&amp;order=uid"><img src="<{$images_url}>/up.gif" border="0" align="middle" alt="" /></a><a href="<{$get_target}>?smode=List&amp;cid=<{$now_cid}>&amp;caldate=<{$caldate}>&amp;op=<{$op}>&amp;order=uid+DESC"><img src="<{$images_url}>/down.gif" border="0" align="middle" alt="" /></a>)
        <br />
  ...
  <tr>
    <td width="100%" align="center" valign="top">
      ...
      <table width='100%' class='outer' cellpadding='4' cellspacing='1'>
        <tr valign='middle'>
          <th class='head'></th>
          <{if $isAdmin || $showSubmitter}><th class='head'><{$lang_submitter}></th><{/if}>
          <th class='head'><{$lang_startdatetime}></th>
          <th class='head'><{$lang_enddatetime}></th>
          <th class='head'><{$lang_summary}></th>
          <{if $isAdmin}><th class='head'><{$lang_rrule}></th><{/if}>
          <{if $isAdmin}><th class='head'><{$lang_admission}></th><{/if}>
        ...
        <tr>
          ...
          <{if $isAdmin || $showSubmitter}><td class='<{$event.oddeven}>'><{$event.submitter_info}></td><{/if}>
          ...
          <td class='<{$event.oddeven}>'>
            ...
            <!--added by goffy-->
            <{if $event.regonline == 1}>&nbsp;<img src="<{$ro_image}>" height="15px" alt="<{$smarty.const._APCAL_RO_ONLINE_POSS}>" title="<{$smarty.const._APCAL_RO_ONLINE_POSS}>"><{/if}>
          </td>
          <{if $isAdmin}><td class='<{$event.oddeven}>'><{$event.rrule}></td><{/if}>
          <{if $isAdmin}><td class='<{$event.oddeven}>' align='center'>
            <{$event.admission}>
        ...
        <tr>
          <td colspan='7' align='right' class='head'>
            <{$lang_checkeditems}> &nbsp; <{$lang_icalendar_output}><input type='submit' name='output_ics_confirm' value='<{$lang_button_export}>' />
          </td>
        </tr>
        <{/if}>
      ...
      <table border='0' cellspacing='0' cellpadding='0' width='570'>
        ...
        <tr>
          <td width='10'><img src='<{$images_url}>/spacer.gif' alt='' width='10' height='10' /></td>
          <td width='100%' align='right'><a href="<{$print_link}>" target="_blank"><img src="<{$images_url}>/print.gif" alt="<{$alt_print}>" title="<{$alt_print}>" border="0" /></a></td>
        </tr>
        <{/if}>
      </table>
